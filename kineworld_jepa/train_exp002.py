"""KINE-EXP-002 trainer: freeze encoder, train intervention head.

  python -m kineworld_jepa.train_exp002 --smoke --steps 30
  python -m kineworld_jepa.train_exp002 --resume PATH --arm C --steps 4000
"""
...
def intervention_ids(arm, batch, device):
    if arm == "A":
        return torch.zeros(batch, dtype=torch.long, device=device)
    if arm == "B":
        return torch.randint(0, NUM_INTERVENTIONS, (batch,), device=device)
...
def main():
    ...
    if args.tiny or (args.smoke and device.type == "cpu"):
        base = KineJEPA(img_size=64, num_frames=4, tubelet_t=2, patch_size=16, enc_depth=1, enc_dim=64, enc_heads=4, pred_depth=2, pred_dim=64, pred_heads=4)
        args.img_size, args.num_frames = 64, 4
    else:
        base = KineJEPA(img_size=args.img_size, num_frames=args.num_frames, enc_depth=args.enc_depth, pred_depth=args.pred_depth)
    if args.resume:
    ...
    print(f"[exp002] arm={args.arm} trainable={sum(p.numel() for p in model.trainable_parameters())} device={device}")
    ...
    for step in range(args.steps):
        ...
        iid = intervention_ids(args.arm, video.shape[0], device)
        ...
        if (step + 1) % args.log_every == 0 or step == 0:
            rec = {"step": step + 1, "loss": round(acc / max(n,1), 6), "arm": args.arm, "mask_ratio": round(float(ratio), 4), "wall_s": round(time.time() - t0, 1)}
